...
public class AccountRepositoryImpl extends MappEntitiesAbstract implements AccountRepository {
	...
	@Override
	public Accounts findAccounts(List<Long> accountIds) {
		Session session = this.sessionFactory.getCurrentSession();
		Cache cache = sessionFactory.getCache();
		for (Long accountId : accountIds) {
			boolean contain = cache.containsEntity(Account.class, accountId);
			if (contain) {
				logger.debug("The cache contains account for key " + accountId);
			} else {
				logger.debug("The cache does not contain account for key " + accountId);
			}
		}
		...
		Accounts accounts = mapAccounts(accountsList, true);
		tx.commit();
		return accounts;
	}
