...
public class BankRepositoryImpl extends MappEntitiesAbstract implements BankRepository {
	...
	public Banks findBanks(List<Long> bankIds) {
		...
		for (Long bankId : bankIds) {
			boolean contain = cache.containsEntity(Bank.class, bankId);
			if (contain) {
				logger.debug("The cache contains bank for key " + bankId);
			} else {
				logger.debug("The cache does not contain bank for key " + bankId);
			}
		}
		Transaction tx = session.beginTransaction();
		Query res = session.createQuery("FROM Bank b WHERE b.bankId IN :bankIds").setParameterList("bankIds", bankIds);
		List<Bank> banksList = res.setCacheable(true).list();
		Banks banks = mapBanks(banksList, true, true);
		tx.commit();
		return banks;
	}
